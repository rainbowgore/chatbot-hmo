...
class ChatInterface:
    ...
    def _render_welcome_message(self):
        ...
        st.info(wrap_rtl_content(welcome_text) if is_rtl_text(welcome_text) else welcome_text, 
               icon="👋")
    ...
    def _render_chat_container(self):
        ...
        with chat_container:
            if not conversation:
                ...
                if language == "he":
                    initial_msg = "שאל אותי כל שאלה רפואית והיא תענה בהתבסס על הפרופיל שלך והידע הרפואי שלי."
                else:
