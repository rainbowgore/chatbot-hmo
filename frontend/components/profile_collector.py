    ...
    def _render_manual_edit_form(self):
        ...
        with st.form("manual_edit_form"):
            ...
            for field, label in field_labels.items():
                ...
                updated_data[field] = st.text_input(label, value=current_value, key=f"edit_{field}")
            
            # Gender selection
            gender_options = ["זכר", "נקבה", "אחר"] if language == "he" else ["Male", "Female", "Other"]
            ...
            updated_data["gender"] = st.selectbox(
                field_labels["gender"], 
                gender_options, 
                index=gender_index,
            ...
            if current_hmo in hmo_options:
                hmo_index = hmo_options.index(current_hmo)
            else:
                hmo_index = 0
            
            updated_data["hmo"] = st.selectbox(
                field_labels["hmo"],
                hmo_options,
                index=hmo_index,
                key="edit_hmo_select"
